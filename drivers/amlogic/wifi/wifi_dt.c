...
int pwm_double_channel_conf_dt(struct wifi_plat_info *plat)
{
	 ...
	 pr_info("wifi: pwm_double_channel_conf_dt() start\n");

	 ret = of_property_read_u32(wifinode, "pwm_config", &pwm_phandle);
	 if (ret) {
		 pr_err("wifi: not match wifi_pwm_config node, ret=%d\n", ret);
		 return -1;
	 }
	 pr_info("wifi: found pwm_config phandle=0x%x\n", pwm_phandle);
 
	 pnode = of_find_node_by_phandle(pwm_phandle);
	 if (!pnode) {
		 pr_err("wifi: can't find wifi_pwm_config node\n");
		 return -1;
	 }
	 pr_info("wifi: found pwm_config node: %s\n", pnode->name);
	 ...
	 for_each_child_of_node(pnode, child) {
		 ...
		 pr_info("wifi: requesting PWM%d from child node: %s\n",
			 plat->ddata.num_pwm, child->name);
		 pdata->pwm = devm_of_pwm_get(plat->dev, child, NULL);
		 if (IS_ERR(pdata->pwm)) {
			 ret = PTR_ERR(pdata->pwm);
			 dev_err(plat->dev, "wifi: unable to request PWM%d, ret = %d\n",
				 plat->ddata.num_pwm, ret);
			 return ret;
		 }
		 pr_info("wifi: PWM%d request successful\n", plat->ddata.num_pwm);
...
static int wifi_dev_probe(struct platform_device *pdev)
{
	 ...
	 pr_info("### wifi_dev_probe: START, device=%s ###\n", dev_name(&pdev->dev));
...
#ifdef CONFIG_OF
	 if (pdev->dev.of_node) {
		 pr_info("### wifi_dev_probe: Found OF node ###\n");
...
#ifdef CONFIG_AMLOGIC_PWM_32K
		 WIFI_INFO("CONFIG_AMLOGIC_PWM_32K is enabled\n");
		 if (!of_get_property(pdev->dev.of_node, "disable-wifi-32k", NULL)) {
			 WIFI_INFO("disable-wifi-32k not found, will configure PWM\n");
			 if (of_get_property(pdev->dev.of_node, "single_pwm", NULL)) {
				 ...
				 WIFI_INFO("pwm_double_channel_conf_dt returned %d\n", ret);
				 if (!ret) {
					 ret = pwm_double_channel_conf(plat);
					 WIFI_INFO("pwm_double_channel_conf returned %d\n", ret);
				 } else if (ret == -EPROBE_DEFER) {
					 WIFI_INFO("PWM deferred, will retry later\n");
					 goto out;
				 } else {
					 WIFI_INFO("PWM configuration failed with error %d\n", ret);
				 }
			 }
		 } else {
			 WIFI_INFO("disable-wifi-32k found, skip PWM configuration\n");
		 }
#else
		 WIFI_INFO("CONFIG_AMLOGIC_PWM_32K is NOT enabled\n");
#endif
...
static int __init wifi_dt_init(void)
{
	 ...
	 pr_info("### wifi_dt_init: START ###\n");

#ifdef CONFIG_AMLOGIC_PWM_32K
	 pr_info("wifi_dt: CONFIG_AMLOGIC_PWM_32K is DEFINED at compile time\n");
#else
	 pr_info("wifi_dt: CONFIG_AMLOGIC_PWM_32K is NOT DEFINED at compile time\n");
#endif

	 ret = platform_driver_register(&wifi_plat_driver);
	 pr_info("### wifi_dt_init: platform_driver_register returned %d ###\n", ret);
	 return ret;
}

 /* Use late_initcall to ensure MMC/SDIO controllers are probed first */
