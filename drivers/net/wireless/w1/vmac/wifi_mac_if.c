    ...
    switch (nstate)
    {
        ...
        case WIFINET_S_CONNECTED:
                ...
                && ((wnet_vif->vm_opmode == WIFINET_M_STA) && (wnet_vif->vm_flags_ext & WIFINET_FEXT_SWBMISS)))
            {
                ...
                printk("****************************************************\n");
                /* Reset statistics on new connection for tracking */
                wnet_vif->vm_devstats.tx_dropped = 0;
                wnet_vif->vm_devstats.tx_errors = 0;
                g_auto_gain_base = 0;
            }
...
__unlock:
    ...
    if (VM_WIFI_CONNECT_STATE(nstate)  && !VM_WIFI_CONNECT_STATE(prestate)) {
        ...
        /* Log TX statistics on disconnect for debugging */
        printk("WiFi disconnect stats: tx_packets=%lu, tx_bytes=%lu, tx_failed=%lu, tx_dropped=%lu, tx_errors=%lu\n",
            wnet_vif->vm_devstats.tx_packets, wnet_vif->vm_devstats.tx_bytes,
            wnet_vif->vm_devstats.tx_dropped + wnet_vif->vm_devstats.tx_errors,
            wnet_vif->vm_devstats.tx_dropped, wnet_vif->vm_devstats.tx_errors);
    }
