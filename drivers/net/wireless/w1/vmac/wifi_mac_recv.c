    ...
    struct wifi_station *sta, struct sk_buff *skb,unsigned int channel)
{
...

    {
        ...
        /* Always log disconnect reason for debugging */
        printk("WiFi disconnect: Received DEAUTH from AP, reason_code=%d\n", reason);
    ...
    struct wifi_station *sta, struct sk_buff *skb,unsigned int channel)
{
...

    {
        ...
        /* Always log disconnect reason for debugging */
        printk("WiFi disconnect: Received DISASSOC from AP %02x:%02x:%02x:%02x:%02x:%02x, reason_code=%d\n",
            wh->i_addr2[0], wh->i_addr2[1], wh->i_addr2[2],
            wh->i_addr2[3], wh->i_addr2[4], wh->i_addr2[5], reason);

        /* Removed redundant BSSID prints - already shown above */
...
#endif
    /* Remove P2P action frame logs (category 127) to reduce log noise */
    if (ia->ia_category != 127) {
