...
int mmc_of_parse_voltage(struct mmc_host *host, u32 *mask)
 ...
 struct mmc_host *mmc_alloc_host(int extra, struct device *dev)
 {
	 ...
	 dev_info(dev, "### MMC ALLOC: device=%s, alias_id=%d\n", dev_name(dev), alias_id);
	 
	 if (alias_id >= 0) {
		 index = alias_id;
		 dev_info(dev, "### MMC ALLOC: using alias ID, index=%d (from DT alias)\n", index);
	 } else {
		 ...
		 if (index < 0) {
			 dev_err(dev, "### MMC ALLOC: ida_simple_get failed, index=%d\n", index);
			 kfree(host);
			 return NULL;
		 }
		 dev_info(dev, "### MMC ALLOC: using dynamic ID, index=%d (min_idx=%d)\n", index, min_idx);
	 }

	 host->index = index;

	 dev_set_name(&host->class_dev, "mmc%d", host->index);
	 dev_info(dev, "### MMC ALLOC: final assignment -> mmc%d\n", host->index);
