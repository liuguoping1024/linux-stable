...
int mmc_attach_sdio(struct mmc_host *host)
{
	 ...
	 pr_info("%s: Starting SDIO attach, sending CMD5 to probe...\n", mmc_hostname(host));
	 err = mmc_send_io_op_cond(host, 0, &ocr);
	 if (err) {
		 pr_err("%s: CMD5 failed with error %d\n", mmc_hostname(host), err);
		 return err;
	 }
	 pr_info("%s: CMD5 succeeded, OCR=0x%08x\n", mmc_hostname(host), ocr);
	  ...
	  */
	 if (!rocr) {
		 pr_err("%s: No compatible voltage found, OCR=0x%08x\n", mmc_hostname(host), ocr);
		 err = -EINVAL;
		 goto err;
	 }
	 pr_info("%s: Voltage selected, ROCR=0x%08x\n", mmc_hostname(host), rocr);

	 /*
	  * Detect and init the card.
	  */
	 pr_info("%s: Initializing SDIO card...\n", mmc_hostname(host));
	 err = mmc_sdio_init_card(host, rocr, NULL);
	 if (err) {
		 pr_err("%s: mmc_sdio_init_card failed with error %d\n", mmc_hostname(host), err);
		 goto err;
	 }
	 pr_info("%s: SDIO card initialized successfully\n", mmc_hostname(host));
