...
#define   CLK_IRQ_SDIO_SLEEP(h)		(h->data->irq_sdio_sleep)
 ...
 static int meson_mmc_probe(struct platform_device *pdev)
 {
	 ...
	 dev_info(&pdev->dev, "%s: probe start, device=%s\n", __func__, dev_name(&pdev->dev));
	 ...
	 if (ret) {
		 dev_info(&pdev->dev, "%s: mmc_regulator_get_supply failed: %d\n", __func__, ret);
		 goto free_host;
	 }
	 ...
	 if (ret) {
		 if (ret != -EPROBE_DEFER)
			 ...
			 dev_info(&pdev->dev, "%s: mmc_of_parse returned -EPROBE_DEFER\n", __func__);
		 goto free_host;
	 }
	 ...
	 //这里dump一下no-sdio, no-sd, no-mmc
	 printk("###%s: MMC_CAP2 flags: NO_SDIO=%d, NO_SD=%d, NO_MMC=%d\n",
		mmc_hostname(mmc),
		!!(mmc->caps2 & MMC_CAP2_NO_SDIO),
		!!(mmc->caps2 & MMC_CAP2_NO_SD),
		!!(mmc->caps2 & MMC_CAP2_NO_MMC));

	 /* 只要支持 SDIO 就设置 sdio_host */
	 if (!(mmc->caps2 & MMC_CAP2_NO_SDIO))
	{
		printk("###%s: Set sdio_host (supports SDIO) ###\n",mmc_hostname(mmc));
		sdio_host = mmc;
	} else {
		printk("###%s: Skip sdio_host (NO_SDIO=1) ###\n",mmc_hostname(mmc));
	}
	return 0;
